...
class TestCodeShieldProperties:
    ...
    def test_sanitized_content_never_longer(self, shield, content):
        ...
        result = shield.scan(content)
        assert len(result.sanitized_content) <= len(content) + 100
...
class TestGuardrailIntegrationProperties:
    ...
    @settings(max_examples=50, suppress_health_check=_SUPPRESS, deadline=None)
    ...
    @settings(suppress_health_check=_SUPPRESS, deadline=None)
